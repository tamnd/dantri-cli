...
describes what users get. Pick a format with `-o`, or let dantri choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
dantri <command> -o table   # aligned columns for reading
dantri <command> -o jsonl   # one JSON object per line, for piping
dantri <command> -o json    # a single JSON array
dantri <command> -o csv     # spreadsheet friendly
dantri <command> -o tsv     # tab-separated
dantri <command> -o url     # just the URL column
dantri <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
dantri <command> --fields id,title,url
```
...
```bash
dantri <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
dantri <command>            # a table, because this is a terminal
dantri <command> | wc -l    # JSONL, because this is a pipe
```
